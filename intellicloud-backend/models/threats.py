...
THREAT_TIME_COL = "created_at"


def _isoify_rows(rows: List[dict], field: str) -> List[dict]:
    for r in rows:
...
def get_threats_for_user(user_id: str) -> List[dict]:
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                f"""
                SELECT id, ip_address, threat_level, description, {THREAT_TIME_COL} AS timestamp
                FROM threats
                WHERE user_id = %s
                ORDER BY id DESC
                """,
            ...
            rows = cur.fetchall()
            return _isoify_rows(rows, "timestamp")
...
def insert_threat(
    ...
    try:
        with conn, conn.cursor() as cur:
            cur.execute(
                f"""
                INSERT INTO threats (ip_address, threat_level, description, {THREAT_TIME_COL}, user_id, client_id)
...
def update_threat_by_id(user_id: str, threat_id: int, updates: Dict[str, Any]) -> bool:
    conn = get_db_connection()
    if not conn:
        print("Cannot update threat: no DB connection")
        return False
    ...
    try:
        ...
        print("Failed to update threat:", e)
        return False
...
def get_audit_logs() -> List[dict]:
    conn = get_db_connection()
    if not conn:
        print("Cannot fetch audit logs: no DB connection")
        return []
...
def log_action(action: str, user_id: str, target_id: int | None = None) -> None:
    conn = get_db_connection()
    if not conn:
        print("Cannot log action: no DB connection")
        return
