...
const STATUS_CONFIG = {
    open:         { label: "Open",         badge: "high",  dot: "#ef4444" },
    investigating:{ label: "Investigating",badge: "med",   dot: "#f59e0b" },
    closed:       { label: "Closed",       badge: "low",   dot: "#22c55e" },
};

const PRIORITY_CONFIG = {
    critical: { label: "Critical", badge: "crit" },
    high:     { label: "High",     badge: "high" },
    medium:   { label: "Medium",   badge: "med"  },
...
function CaseDetail({ caseData, onUpdate, onClose }) {
    ...
    <div style={{
        ...
        background: "var(--panel-2)", display: "flex", justifyContent: "space-between", alignItems: "flex-start",
        }}>
        <div style={{ flex: 1, marginRight: 16 }}>
            ...
            <span style={{ fontSize: 11, color: "var(--text-2)", padding: "2px 0" }}>
                #{localCase.id} · {localCase.alert_count} alerts
            </span>
            </div>
            <h3 style={{ margin: 0, fontSize: 15, fontWeight: 700, lineHeight: 1.4 }}>{localCase.title}</h3>
        ...
        <button className="btn" onClick={onClose} style={{ padding: "6px 10px", flexShrink: 0 }}>✕</button>
        </div>
        ...
        {Object.entries(STATUS_CONFIG).map(([key, cfg]) => (
            ...
            style={{
                ...
                background: localCase.status === key ? cfg.dot : undefined,
                color: localCase.status === key ? "#fff" : undefined,
                opacity: localCase.status === key ? 1 : 0.7,
            }}
            >
            {cfg.label}
            </button>
        ...
        <div style={{ padding: "12px 24px", borderBottom: "1px solid var(--border)", fontSize: 13, color: "var(--text-2)" }}>
            {localCase.description}
        </div>
        )}
        ...
        <div style={{ fontWeight: 700, fontSize: 12, color: "var(--brand)", marginBottom: 16, textTransform: "uppercase", letterSpacing: 1 }}>
            Investigation Timeline
        </div>
        {(localCase.events || []).length === 0 ? (
            <div style={{ color: "var(--text-2)", fontSize: 13 }}>No timeline entries yet. Add a note below.</div>
        ) : (
        ...
        {/* Add note */}
        <div style={{ padding: "16px 24px", borderTop: "1px solid var(--border)", background: "var(--panel-2)" }}>
        <div style={{ display: "flex", gap: 8, marginBottom: 8 }}>
            ...
            <button
                ...
                background: noteType === t ? "var(--brand)" : "transparent",
                color: noteType === t ? "#fff" : "var(--text-2)",
                }}
            >
                {t}
            </button>
...
function NewCaseModal({ onClose, onCreate }) {
    ...
    <div style={{
        ...
        <h3 style={{ margin: "0 0 24px", fontSize: 18 }}>Open New Case</h3>

        <div style={{ marginBottom: 16 }}>
            <label className="label">Case Title *</label>
            <input className="input" placeholder="e.g. Suspected RDP brute force from 45.33.32.156"
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 16 }}>
            ...
            <input className="input" placeholder="analyst@example.com"
        ...
        <div style={{ marginBottom: 24 }}>
            ...
            <textarea className="input" rows={3} placeholder="Brief summary of what triggered this case..."
        ...
        <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
            ...
            <button className="btn primary" onClick={handleSubmit} disabled={saving || !form.title.trim()}>
...
export default function Cases() {
    ...
    <div className="shell animate-fade" style={{ maxWidth: 1400 }}>
        ...
        <div>
            <h1 style={{ margin: 0, fontSize: 22, fontWeight: 800 }}>Case Management</h1>
            <p style={{ margin: "4px 0 0", color: "var(--text-2)", fontSize: 14 }}>
            Track and investigate security incidents from alert to resolution.
            </p>
        </div>
        <button className="btn primary" onClick={() => setShowNewModal(true)} style={{ gap: 8 }}>
            + Open New Case
        </button>
        </div>
        ...
        {[
            ...
            <div style={{ fontSize: 12, color: "var(--text-2)", fontWeight: 600, marginTop: 4 }}>{s.label}</div>
            </div>
        ))}
        </div>
        ...
        <input
            className="input" placeholder="Search cases..."
            value={search} onChange={e => setSearch(e.target.value)}
            style={{ width: 260 }}
        />
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)" }}>STATUS:</span>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <span style={{ fontSize: 12, fontWeight: 700, color: "var(--text-2)" }}>PRIORITY:</span>
        ...
        {loading ? (
            <div style={{ padding: 48, textAlign: "center", color: "var(--text-2)" }}>Loading cases...</div>
        ) : filtered.length === 0 ? (
            ...
            <thead>
                ...
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Case</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Status</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Priority</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Alerts</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Assigned</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Updated</th>
                <th style={{ padding: "10px 16px", textAlign: "left", fontSize: 12, fontWeight: 700 }}>Timeline</th>
                </tr>
            </thead>
            <tbody>
                {filtered.map(c => {
                ...
                return (
                    ...
                    <td style={{ padding: "12px 16px", maxWidth: 380 }}>
                        ...
                        {c.description && (
                        <div style={{ fontSize: 12, color: "var(--text-2)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", maxWidth: 360 }}>
                            {c.description}
                        </div>
                        )}
                    </td>
                    ...
                    <td style={{ padding: "12px 16px", fontSize: 12, color: "var(--text-2)", fontFamily: "monospace" }}>
